...
public interface ActivityDAOService {
    ...
    @Select(value = "{CALL add_activity(" + "#{title, mode=IN, jdbcType=VARCHAR, javaType=String},"
            + "#{description, mode=IN, jdbcType=VARCHAR, javaType=String},"
            + "#{ownerId, mode=IN, jdbcType=BIGINT, javaType=Long})}")
    @Results(id = "activity", value = { @Result(property = "creationDate", column = "creation_date"),
